...
export default function CartIcon({ onClick, onMouseEnter, onMouseLeave, className = '' }: CartIconProps) {
  ...
  return (
    <Link
      href="/cart"
      className={`p-2 relative hover:opacity-70 transition-opacity ${className}`}
    ...
    >
      ...
      {itemCount > 0 && (
        <span
          className="absolute top-4 left-2 flex items-center justify-center min-w-[16px] h-4 px-1 rounded-full text-[10px] font-bold text-white"
